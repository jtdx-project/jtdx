...
! Compute sync power for a complex, downsampled FT8 signal.
  ...
  do i=0,6 ! Sum over 7 Costas frequencies and three Costas arrays
    ...
    k=k+32
  enddo
  if(imainpass.eq.1 .or. imainpass.eq.5 .or. imainpass.eq.9) then
  ...
  else if(imainpass.eq.2 .or. imainpass.eq.6 .or. imainpass.eq.7) then
    if(.not.lastsync) then
      do i=0,6
  ...
  else ! if(imainpass.eq.3 .or. imainpass.eq.4 .or. imainpass.eq.8)  then
    do i=0,6
      if(i.lt.6) then
