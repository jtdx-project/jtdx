...
!  use timer_module, only: timer
  ...
  use ft8_mod1, only : ndecodes,allmessages,allsnrs,allfreq,mycall12_0,mycall12_00,hiscall12_0,nmsg,odd,even,oddcopy,   &
                       ...
                       lmultinst,dd8,nft8cycles,nft8swlcycles,lskiptx1,ncandall,ftopthr,fbotthr,flasttopthr,flastbotthr
...
!print *,nuserthr,numcores,numthreads
     ...
     ftopthr=6000.; fbotthr=6000.; flasttopthr=6000.; flastbotthr=6000.
...
!!!  !$omp parallel sections num_threads(2) copyin(/timer_private/) shared(ndecoded,dd8,allmessages,allsnrs,allfreq,ncandall,nmsg,even,odd,lqsomsgdcd,first_osd,gen,ihash22,nzhash,calls10,calls12,calls22,ftopthr,fbotthr) if(.true.) !iif() needed on Mac
  !$omp parallel sections num_threads(2) shared(ndecoded,dd8,allmessages,allsnrs,allfreq,ncandall,nmsg,even,odd,lqsomsgdcd), &
  !$omp& shared(first_osd,gen,ihash22,nzhash,calls10,calls12,calls22,ftopthr,fbotthr) if(.true.) !iif() needed on Mac
...
!!!  !$omp parallel sections num_threads(3) copyin(/timer_private/) shared(ndecoded,dd8,allmessages,allsnrs,allfreq,ncandall,nmsg,even,odd,lqsomsgdcd,first_osd,gen,ihash22,nzhash,calls10,calls12,calls22,ftopthr,fbotthr) if(.true.) !iif() needed on Mac
  !$omp parallel sections num_threads(3) shared(ndecoded,dd8,allmessages,allsnrs,allfreq,ncandall,nmsg,even,odd,lqsomsgdcd), &
  !$omp& shared(first_osd,gen,ihash22,nzhash,calls10,calls12,calls22,ftopthr,fbotthr) if(.true.) !iif() needed on Mac
...
!!!  !$omp parallel sections num_threads(4) copyin(/timer_private/) shared(ndecoded,dd8,allmessages,allsnrs,allfreq,ncandall,nmsg,even,odd,lqsomsgdcd,first_osd,gen,ihash22,nzhash,calls10,calls12,calls22,ftopthr,fbotthr) if(.true.) !iif() needed on Mac
  !$omp parallel sections num_threads(4) shared(ndecoded,dd8,allmessages,allsnrs,allfreq,ncandall,nmsg,even,odd,lqsomsgdcd), &
  !$omp& shared(first_osd,gen,ihash22,nzhash,calls10,calls12,calls22,ftopthr,fbotthr) if(.true.) !iif() needed on Mac
...
!!!  !$omp parallel sections num_threads(5) copyin(/timer_private/) shared(ndecoded,dd8,allmessages,allsnrs,allfreq,ncandall,nmsg,even,odd,lqsomsgdcd,first_osd,gen,ihash22,nzhash,calls10,calls12,calls22,ftopthr,fbotthr) if(.true.) !iif() needed on Mac
  !$omp parallel sections num_threads(5) shared(ndecoded,dd8,allmessages,allsnrs,allfreq,ncandall,nmsg,even,odd,lqsomsgdcd), &
  !$omp& shared(first_osd,gen,ihash22,nzhash,calls10,calls12,calls22,ftopthr,fbotthr) if(.true.) !iif() needed on Mac
...
!!!  !$omp parallel sections num_threads(6) copyin(/timer_private/) shared(ndecoded,dd8,allmessages,allsnrs,allfreq,ncandall,nmsg,even,odd,lqsomsgdcd,first_osd,gen,ihash22,nzhash,calls10,calls12,calls22,ftopthr,fbotthr) if(.true.) !iif() needed on Mac
  !$omp parallel sections num_threads(6) shared(ndecoded,dd8,allmessages,allsnrs,allfreq,ncandall,nmsg,even,odd,lqsomsgdcd), &
  !$omp& shared(first_osd,gen,ihash22,nzhash,calls10,calls12,calls22,ftopthr,fbotthr) if(.true.) !iif() needed on Mac
...
!!!  !$omp parallel sections num_threads(7) copyin(/timer_private/) shared(ndecoded,dd8,allmessages,allsnrs,allfreq,ncandall,nmsg,even,odd,lqsomsgdcd,first_osd,gen,ihash22,nzhash,calls10,calls12,calls22,ftopthr,fbotthr) if(.true.) !iif() needed on Mac
  !$omp parallel sections num_threads(7) shared(ndecoded,dd8,allmessages,allsnrs,allfreq,ncandall,nmsg,even,odd,lqsomsgdcd), &
  !$omp& shared(first_osd,gen,ihash22,nzhash,calls10,calls12,calls22,ftopthr,fbotthr) if(.true.) !iif() needed on Mac
...
!!!  !$omp parallel sections num_threads(8) copyin(/timer_private/) shared(ndecoded,dd8,allmessages,allsnrs,allfreq,ncandall,nmsg,even,odd,lqsomsgdcd,first_osd,gen,ihash22,nzhash,calls10,calls12,calls22,ftopthr,fbotthr) if(.true.) !iif() needed on Mac
  !$omp parallel sections num_threads(8) shared(ndecoded,dd8,allmessages,allsnrs,allfreq,ncandall,nmsg,even,odd,lqsomsgdcd), &
  !$omp& shared(first_osd,gen,ihash22,nzhash,calls10,calls12,calls22,ftopthr,fbotthr) if(.true.) !iif() needed on Mac
...
!!!  !$omp parallel sections num_threads(9) copyin(/timer_private/) shared(ndecoded,dd8,allmessages,allsnrs,allfreq,ncandall,nmsg,even,odd,lqsomsgdcd,first_osd,gen,ihash22,nzhash,calls10,calls12,calls22,ftopthr,fbotthr) if(.true.) !iif() needed on Mac
  !$omp parallel sections num_threads(9) shared(ndecoded,dd8,allmessages,allsnrs,allfreq,ncandall,nmsg,even,odd,lqsomsgdcd), &
  !$omp& shared(first_osd,gen,ihash22,nzhash,calls10,calls12,calls22,ftopthr,fbotthr) if(.true.) !iif() needed on Mac
...
!!!  !$omp parallel sections num_threads(10) copyin(/timer_private/) shared(ndecoded,dd8,allmessages,allsnrs,allfreq,ncandall,nmsg,even,odd,lqsomsgdcd,first_osd,gen,ihash22,nzhash,calls10,calls12,calls22,ftopthr,fbotthr) if(.true.) !iif() needed on Mac
  !$omp parallel sections num_threads(10) shared(ndecoded,dd8,allmessages,allsnrs,allfreq,ncandall,nmsg,even,odd,lqsomsgdcd), &
  !$omp& shared(first_osd,gen,ihash22,nzhash,calls10,calls12,calls22,ftopthr,fbotthr) if(.true.) !iif() needed on Mac
...
!!!  !$omp parallel sections num_threads(11) copyin(/timer_private/) shared(ndecoded,dd8,allmessages,allsnrs,allfreq,ncandall,nmsg,even,odd,lqsomsgdcd,first_osd,gen,ihash22,nzhash,calls10,calls12,calls22,ftopthr,fbotthr) if(.true.) !iif() needed on Mac
  !$omp parallel sections num_threads(11) shared(ndecoded,dd8,allmessages,allsnrs,allfreq,ncandall,nmsg,even,odd,lqsomsgdcd), &
  !$omp& shared(first_osd,gen,ihash22,nzhash,calls10,calls12,calls22,ftopthr,fbotthr) if(.true.) !iif() needed on Mac
...
!!!  !$omp parallel sections num_threads(12) copyin(/timer_private/) shared(ndecoded,dd8,allmessages,allsnrs,allfreq,ncandall,nmsg,even,odd,lqsomsgdcd,first_osd,gen,ihash22,nzhash,calls10,calls12,calls22,ftopthr,fbotthr) if(.true.) !iif() needed on Mac
  !$omp parallel sections num_threads(12) shared(ndecoded,dd8,allmessages,allsnrs,allfreq,ncandall,nmsg,even,odd,lqsomsgdcd), &
  !$omp& shared(first_osd,gen,ihash22,nzhash,calls10,calls12,calls22,ftopthr,fbotthr) if(.true.) !iif() needed on Mac
...
!!!  !$omp parallel sections num_threads(13) copyin(/timer_private/) shared(ndecoded,dd8,allmessages,allsnrs,allfreq,ncandall,nmsg,even,odd,lqsomsgdcd,first_osd,gen,ihash22,nzhash,calls10,calls12,calls22,ftopthr,fbotthr) if(.true.) !iif() needed on Mac
  !$omp parallel sections num_threads(13) shared(ndecoded,dd8,allmessages,allsnrs,allfreq,ncandall,nmsg,even,odd,lqsomsgdcd), &
  !$omp& shared(first_osd,gen,ihash22,nzhash,calls10,calls12,calls22,ftopthr,fbotthr) if(.true.) !iif() needed on Mac
...
!!!  !$omp parallel sections num_threads(14) copyin(/timer_private/) shared(ndecoded,dd8,allmessages,allsnrs,allfreq,ncandall,nmsg,even,odd,lqsomsgdcd,first_osd,gen,ihash22,nzhash,calls10,calls12,calls22,ftopthr,fbotthr) if(.true.) !iif() needed on Mac
  !$omp parallel sections num_threads(14) shared(ndecoded,dd8,allmessages,allsnrs,allfreq,ncandall,nmsg,even,odd,lqsomsgdcd), &
  !$omp& shared(first_osd,gen,ihash22,nzhash,calls10,calls12,calls22,ftopthr,fbotthr) if(.true.) !iif() needed on Mac
...
!!!  !$omp parallel sections num_threads(15) copyin(/timer_private/) shared(ndecoded,dd8,allmessages,allsnrs,allfreq,ncandall,nmsg,even,odd,lqsomsgdcd,first_osd,gen,ihash22,nzhash,calls10,calls12,calls22,ftopthr,fbotthr) if(.true.) !iif() needed on Mac
  !$omp parallel sections num_threads(15) shared(ndecoded,dd8,allmessages,allsnrs,allfreq,ncandall,nmsg,even,odd,lqsomsgdcd), &
  !$omp& shared(first_osd,gen,ihash22,nzhash,calls10,calls12,calls22,ftopthr,fbotthr) if(.true.) !iif() needed on Mac
...
!!!  !$omp parallel sections num_threads(16) copyin(/timer_private/) shared(ndecoded,dd8,allmessages,allsnrs,allfreq,ncandall,nmsg,even,odd,lqsomsgdcd,first_osd,gen,ihash22,nzhash,calls10,calls12,calls22,ftopthr,fbotthr) if(.true.) !iif() needed on Mac
  !$omp parallel sections num_threads(16) shared(ndecoded,dd8,allmessages,allsnrs,allfreq,ncandall,nmsg,even,odd,lqsomsgdcd), &
  !$omp& shared(first_osd,gen,ihash22,nzhash,calls10,calls12,calls22,ftopthr,fbotthr) if(.true.) !iif() needed on Mac
...
!!!  !$omp parallel sections num_threads(17) copyin(/timer_private/) shared(ndecoded,dd8,allmessages,allsnrs,allfreq,ncandall,nmsg,even,odd,lqsomsgdcd,first_osd,gen,ihash22,nzhash,calls10,calls12,calls22,ftopthr,fbotthr) if(.true.) !iif() needed on Mac
  !$omp parallel sections num_threads(17) shared(ndecoded,dd8,allmessages,allsnrs,allfreq,ncandall,nmsg,even,odd,lqsomsgdcd), &
  !$omp& shared(first_osd,gen,ihash22,nzhash,calls10,calls12,calls22,ftopthr,fbotthr) if(.true.) !iif() needed on Mac
!     call omp_set_num_threads(17)
  !$omp section
     call my_ft8%decode(ft8_decoded,params%nQSOProgress,nfqso,params%nft8rxfsens,  &
          params%nftx,nutc,nfmid7+1,nfmid8,params%ncandthin,params%ndtcenter, &
          logical(params%nhint),nsec,params%napwid,params%nswl,params%lmycallstd,params%lhiscallstd, &
...
!!!  !$omp parallel sections num_threads(18) copyin(/timer_private/) shared(ndecoded,dd8,allmessages,allsnrs,allfreq,ncandall,nmsg,even,odd,lqsomsgdcd,first_osd,gen,ihash22,nzhash,calls10,calls12,calls22,ftopthr,fbotthr) if(.true.) !iif() needed on Mac
  !$omp parallel sections num_threads(18) shared(ndecoded,dd8,allmessages,allsnrs,allfreq,ncandall,nmsg,even,odd,lqsomsgdcd), &
  !$omp& shared(first_osd,gen,ihash22,nzhash,calls10,calls12,calls22,ftopthr,fbotthr) if(.true.) !iif() needed on Mac
...
!!!  !$omp parallel sections num_threads(19) copyin(/timer_private/) shared(ndecoded,dd8,allmessages,allsnrs,allfreq,ncandall,nmsg,even,odd,lqsomsgdcd,first_osd,gen,ihash22,nzhash,calls10,calls12,calls22,ftopthr,fbotthr) if(.true.) !iif() needed on Mac
  !$omp parallel sections num_threads(19) shared(ndecoded,dd8,allmessages,allsnrs,allfreq,ncandall,nmsg,even,odd,lqsomsgdcd), &
  !$omp& shared(first_osd,gen,ihash22,nzhash,calls10,calls12,calls22,ftopthr,fbotthr) if(.true.) !iif() needed on Mac
!     call omp_set_num_threads(19)
  !$omp section
     call my_ft8%decode(ft8_decoded,params%nQSOProgress,nfqso,params%nft8rxfsens,  &
          params%nftx,nutc,nfmid8+1,nfmid9,params%ncandthin,params%ndtcenter, &
          logical(params%nhint),nsec,params%napwid,params%nswl,params%lmycallstd,params%lhiscallstd, &
...
!!!  !$omp parallel sections num_threads(20) copyin(/timer_private/) shared(ndecoded,dd8,allmessages,allsnrs,allfreq,ncandall,nmsg,even,odd,lqsomsgdcd,first_osd,gen,ihash22,nzhash,calls10,calls12,calls22,ftopthr,fbotthr) if(.true.) !iif() needed on Mac
  !$omp parallel sections num_threads(20) shared(ndecoded,dd8,allmessages,allsnrs,allfreq,ncandall,nmsg,even,odd,lqsomsgdcd), &
  !$omp& shared(first_osd,gen,ihash22,nzhash,calls10,calls12,calls22,ftopthr,fbotthr) if(.true.) !iif() needed on Mac
...
!!!  !$omp parallel sections num_threads(21) copyin(/timer_private/) shared(ndecoded,dd8,allmessages,allsnrs,allfreq,ncandall,nmsg,even,odd,lqsomsgdcd,first_osd,gen,ihash22,nzhash,calls10,calls12,calls22,ftopthr,fbotthr) if(.true.) !iif() needed on Mac
  !$omp parallel sections num_threads(21) shared(ndecoded,dd8,allmessages,allsnrs,allfreq,ncandall,nmsg,even,odd,lqsomsgdcd), &
  !$omp& shared(first_osd,gen,ihash22,nzhash,calls10,calls12,calls22,ftopthr,fbotthr) if(.true.) !iif() needed on Mac
!     call omp_set_num_threads(21)
  !$omp section
     call my_ft8%decode(ft8_decoded,params%nQSOProgress,nfqso,params%nft8rxfsens,  &
          params%nftx,nutc,nfmid9+1,nfmid10,params%ncandthin,params%ndtcenter, &
          logical(params%nhint),nsec,params%napwid,params%nswl,params%lmycallstd,params%lhiscallstd, &
...
!!!  !$omp parallel sections num_threads(22) copyin(/timer_private/) shared(ndecoded,dd8,allmessages,allsnrs,allfreq,ncandall,nmsg,even,odd,lqsomsgdcd,first_osd,gen,ihash22,nzhash,calls10,calls12,calls22,ftopthr,fbotthr) if(.true.) !iif() needed on Mac
  !$omp parallel sections num_threads(22) shared(ndecoded,dd8,allmessages,allsnrs,allfreq,ncandall,nmsg,even,odd,lqsomsgdcd), &
  !$omp& shared(first_osd,gen,ihash22,nzhash,calls10,calls12,calls22,ftopthr,fbotthr) if(.true.) !iif() needed on Mac
...
!!!  !$omp parallel sections num_threads(23) copyin(/timer_private/) shared(ndecoded,dd8,allmessages,allsnrs,allfreq,ncandall,nmsg,even,odd,lqsomsgdcd,first_osd,gen,ihash22,nzhash,calls10,calls12,calls22,ftopthr,fbotthr) if(.true.) !iif() needed on Mac
  !$omp parallel sections num_threads(23) shared(ndecoded,dd8,allmessages,allsnrs,allfreq,ncandall,nmsg,even,odd,lqsomsgdcd), &
  !$omp& shared(first_osd,gen,ihash22,nzhash,calls10,calls12,calls22,ftopthr,fbotthr) if(.true.) !iif() needed on Mac
!     call omp_set_num_threads(23)
  !$omp section
     call my_ft8%decode(ft8_decoded,params%nQSOProgress,nfqso,params%nft8rxfsens,  &
          params%nftx,nutc,nfmid10+1,nfmid11,params%ncandthin,params%ndtcenter, &
          logical(params%nhint),nsec,params%napwid,params%nswl,params%lmycallstd,params%lhiscallstd, &
...
!!!  !$omp parallel sections num_threads(24) copyin(/timer_private/) shared(ndecoded,dd8,allmessages,allsnrs,allfreq,ncandall,nmsg,even,odd,lqsomsgdcd,first_osd,gen,ihash22,nzhash,calls10,calls12,calls22,ftopthr,fbotthr) if(.true.) !iif() needed on Mac
  !$omp parallel sections num_threads(24) shared(ndecoded,dd8,allmessages,allsnrs,allfreq,ncandall,nmsg,even,odd,lqsomsgdcd), &
  !$omp& shared(first_osd,gen,ihash22,nzhash,calls10,calls12,calls22,ftopthr,fbotthr) if(.true.) !iif() needed on Mac
