! This source code file was last time modified by Igor UA3DJY on May 29th, 2017
! All changes are shown in the patch file coming together with the full JTDX source code.

subroutine subtract10(i4Decoded6BitWords,freq,drift,xdt,i4GrayCodedSym,hintdec)

! Subtract a jt10 signal
!
...
!  use timer_module, only: timer
  use packjt
  use jt65_mod6 ! real*4 dd(NPTS) NPTS=624000
...
! Insert sync symbols at ntone=0 and add 2 to the data-tone numbers.
  ...
  do i=1,85
     if(isync(i).eq.1 .or. isync(i).eq.2) then
        if(isync(i).eq.1) i4tone(i)=0
        if(isync(i).eq.2) i4tone(i)=1
     else
        j=j+1
        i4tone(i)=i4GrayCodedSymbols(j)+2
     endif
  enddo
...
!  call timer('subtr_1 ',0)
  ...
  a2=drift/(-2.0)
  do k=1,nsym
    omega=(pi512*(freq+a2*(k*ns+x1-x0)/653184.0+6.94444444*i4tone(isym)))/256 ! =2*pi*(f0+...
    isym=isym+1
    dphi=omega/12000.0
    do i=1,ns
