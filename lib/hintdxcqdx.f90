...
subroutine hintdxcqdx(mrs,mrs2,hiscall,hisgrid,decoded,hint_dx)
  ...
  character msg*22,msg1*22,msg00*22,decoded*22,hisgrid*4,hiscall*6,prevcall*6
  ...
  if(first) then
     ...
     enddo
     do m=11,MAXMSG
        msg='CQ UF1XYZ '//'KO8'//achar(47+m-10)
        call fmtmsg(msg,iz)
        call packmsg(msg,dgen,itype)            !Pack message into 72 bits
        call rs_encode(dgen,sym_rev)            !RS encode
        call interleave63(sym_rev,1)            !Interleave channel symbols
...
! a bank of matched filters on the symbol spectra s3(i,j).
  ...
  decoded='                      '
  msg1=msg0(ipk)
  if(msg1(1:9).eq."CQ UF0XYZ" .or. msg1(1:9).eq."CQ UF1XYZ") go to 4
