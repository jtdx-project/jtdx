...
!if(nft.eq.0) then ! diag only
  if(.not.stophint .and. hint .and. nft.eq.0 .and. snrdb.lt.hintth) then
     if(rxfreq) then
        if(nlasttx.eq.1) then
           call hintdxr10(mrs,mrs2,mycall,hiscall,decoded,hintdec,s3,i4GrayCodedSym)
           if(hintdec) go to 2
        endif
        if(nlasttx.eq.2) then
           call hintdxrr10(mrs,mrs2,mycall,hiscall,decoded,hintdec,s3,i4GrayCodedSym)
           if(hintdec) go to 2
           call hintdxgrid10(mrs,mrs2,mycall,hiscall,hisgrid,decoded,hintdec,s3,i4GrayCodedSym)
           if(hintdec) go to 2
        endif
        if(nlasttx.eq.3) then
           call hintdx7310(mrs,mrs2,mycall,hiscall,decoded,hintdec,s3,i4GrayCodedSym)
           if(hintdec) go to 2
           call hintdxr10(mrs,mrs2,mycall,hiscall,decoded,hintdec,s3,i4GrayCodedSym)
           if(hintdec) go to 2
        endif
        ...
        if(nlasttx.eq.6) then !CALL3 based decoder mycall+hiscall+grid message
           call hintrxgrid10(mrs,mrs2,mycall,decoded,hintdec,s3,i4GrayCodedSym)
           if(hintdec) go to 2
        endif
     endif
2    if(hintdec) then 
