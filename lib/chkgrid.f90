...
if(grid(1:1).gt.'@' .and. grid(1:1).lt.'J') then
  ...
  else if(grid(1:1).eq.'I' .and. ((grid(2:2).gt.'@' .and. grid(2:2).lt.'F') .or. &
  ...
  if(grid(1:1).eq.'J' .and. ((grid(2:2).gt.'@' .and. grid(2:2).lt.'D') .or. (grid(2:2).eq.'D' .and. grid(3:4).ne.'15') .or. &
      grid(2:2).eq.'E' .or. (grid(2:2).eq.'F' .and. grid(3:3).lt.'8') .or. (grid(2:2).eq.'G' .and. grid(3:3).lt.'6') .or. &
      (grid(2:2).eq.'H' .and. grid(3:3).lt.'5') .or. (grid(2:2).eq.'I' .and. grid(3:3).lt.'2') .or. &
      (grid(2:2).eq.'R' .and. grid(4:4).gt.'0'))) then; lchkcall=.true.

  else if(grid(1:1).eq.'K' .and. ((grid(2:2).gt.'@' .and. grid(2:2).lt.'E') .or. &
...
! USA AA..AL,K,N,W
    if(callsign(2:2).gt.'@' .and. callsign(2:2).lt.'M') then
      if((grid(1:2).eq.'FM' .and. grid(3:3).gt.'/' .and. grid(3:3).lt.'3' .and. grid(4:4).gt.'1' .and. grid(4:4).lt.':').or. &
       ...
       grid(1:4).eq.'AK56' .or. grid(1:4).eq.'AH48') lgvalid=.true.
...
! Canada CF..CK,CY,CZ,VA..VG,VO,VX,VY,XJ..XO
      ...
      if((grid(1:2).eq.'CN' .and. grid(3:3).gt.'5' .and. grid(3:3).lt.':' .and. grid(4:4).gt.'7' .and. grid(4:4).lt.':').or. &
         ...
         grid(1:2).eq.'CP' .or. grid(1:2).eq.'DP' .or. grid(1:2).eq.'EP' .or. grid(1:4).eq.'GN03' .or. grid(1:4).eq.'FN93') &
        lgvalid=.true.
...
! Portugal, Madeira, Azores CQ..CU
      ...
      if(grid(1:3).eq.'IN5' .or. grid(1:3).eq.'IN6' .or. grid(1:3).eq.'IM5' .or. grid(1:3).eq.'IM6' .or. &
         grid(1:4).eq.'IM12' .or. grid(1:4).eq.'IM13' .or. &
         (grid(1:2).eq.'HM' .and. grid(3:3).gt.'3' .and. grid(3:3).lt.'8')) lgvalid=.true.
...
! not valid callsigns
    else if(callsign(1:1).eq.'C' .and. (callsign(2:2).eq.'0' .or. callsign(2:2).eq.'1' .or. callsign(2:2).eq.'7')) then
...
! Fed. Rep. of Germany DA..DR, Y2..Y9
    if(callsign(1:1).eq.'D' .and. callsign(2:2).gt.'@' .and. callsign(2:2).lt.'S') then
...
! France F,HW..HY,TH,TM,TP,TQ,TV
    if(callsign(2:2).gt.'/' .and. callsign(2:2).lt.':') then
      if((grid(1:2).eq.'IN' .and. grid(3:3).gt.'7' .and. grid(3:3).lt.':' .and. grid(4:4).gt.'2' .and. grid(4:4).lt.':').or. &
         ...
         grid(1:3).eq.'JN02'  .or. grid(1:4).eq.'JN12' .or. grid(1:4).eq.'JO00' .or. grid(1:4).eq.'JO10' .or. &
         grid(1:4).eq.'JO20' .or. grid(1:4).eq.'JO11' .or. grid(1:4).eq.'IN92') lgvalid=.true.
...
! Japan 7J..7N,8J..8N,JA,JE..JS
    if(callsign(1:2).eq.'JA' .or. (callsign(1:1).eq.'J' .and. callsign(2:2).gt.'D' .and. callsign(2:2).lt.'T')) then
...
! Kazakhstan UN..UQ
      ...
      if((grid(1:2).eq.'MN' .and. grid(4:4).gt.'1' .and. grid(4:4).lt.':') .or. &
         ...
         (grid(1:3).eq.'MN' .and. ((grid(4:4).eq.'1' .and. (grid(3:3).eq.'3' .or.grid(3:3).eq.'4'.or.grid(3:3).eq.'5')).or. &
          grid(3:4).eq.'40')) .or. &
