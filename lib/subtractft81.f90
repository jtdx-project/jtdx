...
! Subtract         : dd(t)    = dd(t) - 2*REAL{cref*cfilt}
  ...
  use ft8_mod1, only : dd8,cw,cfilt1,NFILT1,NFILT2,endcorr,endcorrswl
...
!$omp critical(subtraction)
  do i=1,nframe
     j=nstart+i-1
     if(j.ge.1 .and. j.le.NMAX) dd8(j)=dd8(j)-2*REAL(cfilt1(i)*cref(i))
  enddo
