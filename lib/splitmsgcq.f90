! This source code file was last time modified by Igor UA3DJY on January 14th, 2017
...
subroutine splitmsgcq(decoded,dtx)
  ...
  if(i2.eq.3) then
     ...
     i3=i3+i2
if(len_trim(decoded(i2+1:i3-1)).gt.6) go to 4
     callsign=decoded(i2+1:i3-1)
     grid=decoded(i3+1:i3+4)
     go to 2
  endif
...
2 do i=1,300
    if(dynhintcq(i)%ninterval.lt.0) then
       dynhintcq(i)%ninterval=ninterval
       dynhintcq(i)%direction=direction
       dynhintcq(i)%callsign=callsign
       dynhintcq(i)%grid=grid
       dynhintcq(i)%freq=freq
       dynhintcq(i)%dt=dtx
       exit
    endif
  enddo
