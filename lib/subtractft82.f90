subroutine subtractft82(itone,f0,dt,swl)
  ...
  use ft8_mod1, only : dd8,cw,cfilt2,NFILT1,NFILT2,endcorr,endcorrswl
...
!$omp critical(subtraction)
  do i=1,nframe
     j=nstart+i-1
     if(j.ge.1 .and. j.le.NMAX) dd8(j)=dd8(j)-2*REAL(cfilt2(i)*cref(i))
  enddo
